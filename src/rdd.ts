...
const binaryTypes: { [key: string]: BinaryType } = {
  WindowsPlayer: {
    versionFile: "/version",
    blobDir: "/",
  },
...
const consoleTextTest = document.getElementById("consoleText");
const downloadFormTest = document.getElementById("downloadForm");
const downloadFormDivTest = document.getElementById("downloadFormDiv");

if (!consoleTextTest || !downloadFormTest || !downloadFormDivTest) {
  throw new Error("Failed to get elements");
}

const consoleText = consoleTextTest as HTMLElement;
const downloadForm = downloadFormTest as HTMLFormElement;
const downloadFormDiv = downloadFormDivTest as HTMLElement;

function getLinkFromForm() {
  const channelName =
    downloadForm.channel.value.trim() || downloadForm.channel.placeholder;
  let queryString = `?channel=${encodeURIComponent(
    channelName
  )}&binaryType=${encodeURIComponent(downloadForm.binaryType.value)}`;
...
// needs to be exposed via window for the HTML onclick

// @ts-expect-error: This is a function that's called from the HTML
window.downloadFromForm = downloadFromForm;
// @ts-expect-error: This is a function that's called from the HTML
window.copyLinkFromForm = copyLinkFromForm;
 ...
 * @param originalText
 * @returns A HMTL escaped String
 */
function escHtml(originalText: string): string {
  return originalText
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function request(
  url: string,
  callback: (responseText: string, statusCode?: number) => void,
  errorOnNotOk = true
) {
  ...
  httpRequest.onerror = function (_e) {
...
function requestBinary(
  url: string,
  callback: (buffer: ArrayBuffer, statusCode?: number) => void
) {
  const httpRequest = new XMLHttpRequest();
...
const binaryType = getQuery("binaryType");
let blobDir = getQuery("blobDir");

let compressZip: string | boolean | null = getQuery("compressZip");
let compressionLevel: string | number | null = getQuery("compressionLevel");

let channelPath: string;
let versionPath: string;

let extractRoots: { [key: string]: string };
let zip: JSZip;
...
function main() {
  ...
  if (compressionLevel) {
    try {
      compressionLevel = parseInt(compressionLevel as string);
    } catch (err) {
      log(
        `[!] Error: Failed to parse \`compressionLevel\` query: ${err}`,
        "\n\n"
      );
      log(usageMsg, "\n", false);
      return;
  ...
  // @ts-ignore We're only using the binaryType for the version file path
  let versionFilePath: string; // Only used if `version` isn't already defined (later, see code below the if-else after this)
...
async function downloadZipsFromManifest(manifestBody: string) {
  const pkgManifestLines = manifestBody.split("\n").map((line) => line.trim());

  if (pkgManifestLines[0] !== "v0") {
    log(
      `[!] Error: rbxPkgManifest manifest version incorrect; expected "v0", got "${pkgManifestLines[0]}"`
    );
    return;
  }
...
`
  ...
  function checkIfNoThreadsLeft() {
    ...
    if (compressZip) {
      log(
        `[!] NOTE: Compressing final zip (with a compression level of ${compressionLevel}/9), this may take a bit longer than with no compression..`
      );
    }
    ...
    zip
      .generateAsync({
        ...
        compressionOptions: {
          level: compressionLevel as number,
        },
      })
...
async function downloadPackage(
  ...
  requestBinary(blobUrl, async function (blobData) {
    ...
    if (packageName in extractRoots == false) {
      log(
        `[*] Package name "${packageName}" not defined in extraction roots for BinaryType \`${binaryType}\`, skipping extraction! (THIS MAY MAKE THE ZIP OUTPUT INCOMPLETE, BE AWARE!)`
      );
    ...
    await JSZip.loadAsync(blobData).then(async function (packageZip) {
      const fileGetPromises: Promise<any>[] = [];
